...
public class GoldenHopperBlock extends BlockWithEntity {
	...
	public <T extends BlockEntity> BlockEntityTicker<T> getTicker(World world, BlockState state, BlockEntityType<T> type) {
		return world.isClient ? null : checkType(type, FHBlock.GOLDEN_HOPPER_BLOCK_ENTITY, GoldenHopperBlockEntity::serverTick);
	}
	...
	public void onPlaced(World world, BlockPos pos, BlockState state, LivingEntity placer, ItemStack itemStack) {
		if (itemStack.hasCustomName()) {
			...
			if (blockEntity instanceof GoldenHopperBlockEntity) {
				((GoldenHopperBlockEntity)blockEntity).setCustomName(itemStack.getName());
			}
		}
	}
	...
	public ActionResult onUse(BlockState state, World world, BlockPos pos, PlayerEntity player, Hand hand, BlockHitResult hit) {
		if (world.isClient) {
			...
			BlockEntity blockEntity = world.getBlockEntity(pos);
			if (blockEntity instanceof GoldenHopperBlockEntity) {
				player.openHandledScreen((GoldenHopperBlockEntity)blockEntity);
	...
	public void onStateReplaced(BlockState state, World world, BlockPos pos, BlockState newState, boolean moved) {
		if (!state.isOf(newState.getBlock())) {
			BlockEntity blockEntity = world.getBlockEntity(pos);
			if (blockEntity instanceof GoldenHopperBlockEntity) {
				ItemScatterer.spawn(world, pos, (GoldenHopperBlockEntity)blockEntity);
